...
常见的解决方法是使用 `IIFE` - an Immediately Invoked Function Expression，每一次迭代的时候持有变量 i。
<!--JavaScript-->
...
它有点像 `let`，但是它的名字就暗示着变量一旦被绑定就不能再更改，也就是说，它和 `let` 一样，有同样的作用域规则，但是你不能对这样的变量再次赋值。
<!--JavaScript-->
...
你可以采用特殊的方法避免这个限制，`const` 定义的变量其内部状态依然是可以修改的，幸运的是，TypeScript 允许你指定一个对象的成员变量为 `readonly`，[接口](...)一章会有详细描述。

## let vs const

鉴于我们有两种具有相似的作用域语义的变量声明方式，那很自然可以想到，我们该使用哪一种呢？就像大多数问题一样，答案是：看具体情况。

对应于[最小权限原则](https://en.wikipedia.org/wiki/Principle_of_least_privilege)，除了你准备修改的变量以外，其他所有变量都应该用 `const` 来声明。基本原理是：如果一个变量不需要被写，其他使用这个代码的人就不应该能够写这个变量，同时也要考虑是否真的有必要对这个变量重新赋值。使用 `const` 也可以在分析数据流时让代码变得具有可预测性。

使用最佳实践，如果觉得适用，那么可以和团队成员一起讨论（*~以形成约定*）。
...
就像数组解构一样，你可以对已有的变量直接赋值（*~前提是变量 a 和 b 在之前已经定义*）。
<!--JavaScript-->
...
注意我们使用括号**~包裹**代码，因为 JavaScript 把 `{` 当做一个块的开始。
...
> ~总之，`...` 语法定义的变量在 `=` 的左边叫解构，在右边就可以叫重组，后续会讲到这个概念。
...
在上面的代码中，`b?` 表示属性 `b` 是可选的，所以它的值可能是 `undefined`，`keepWholeObject` 函数有一个具有属性 `a` 和 `b` 的参数变量 `wholeObject`，即便 `b` 是 undefined 也没问题。
...
然后，你要记得为非结构化属性的可选项提供一个默认值，记住在类型 C 中 属性 b 是可选的。
<!--JavaScript-->
...
使用解构要小心，就像上面的例子，除了简单的解构表达式外其他的用法都很容易让人混淆，对于深度嵌套的解构更是如此，就算没有重命名、默认值和类型注解都已经很难理解了。要尽量让解构表达式短小和简单。**?You can always write the assignments that destructuring would generate yourself.**

### 重组

重组与解构时相反的，你可以重组一个数组到另外一个数组，或者一个对象到另外一个对象。例如：
<!--JavaScript-->
...
代码设置 `bothPlus` 的值为 `[0, 1, 2, 3, 4, 5]`，重组创建了变量 `first` 和 `second` 的浅拷贝，重组不会改变他们本身的值。
...
现在，`search` 的值为 `{ food: "rich", price: "$$", ambiance: "noisy" }`，对象重组比数组重组更复杂，就像数组重组一样，对象重组从左到右进行处理，但最终的结果仍然是一个对象，这就意味着后续的重组对象中的属性可能会重写之前的重组对象中的属性。我们可以修改之前的例子。
<!--JavaScript-->
...
对象重组也有另外一些意料之外的限制。首先，重组后的对象仅会包含源对象的[可枚举属性](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Enumerability_and_ownership_of_properties)，当完成对象重组后，会丢失源对象拥有的方法。
<!--JavaScript-->
...
其次，TypeScript 编译器不允许重组泛型函数的类型参数，该功能将在未来的版本中提供。
