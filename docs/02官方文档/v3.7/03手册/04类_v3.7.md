...
传统的 JavaScript 使用函数和面向原型的继承方式构建可重用组件，但这种方式可能让已经适应了面向对象编程的程序员感到有些别扭，面向对象编程中的类扩展其他类的功能，对象实例通过类创建。从 ES2015 开始，也就是ES6，JavaScript 程序员也可以使用基于类的面向对象方式构建应用程序。在 TypeScript 中，我们现在已经可以让开发者使用这种技术，然后将代码编译成在各种浏览器和平台上都适用的 JavaScript 代码，不需要等待 JavaScript 的后续版本。
...
在类的内部，当我们使用某个成员的时候在成员前面加 `this.`，这表明在使用内的成员方式进行访问。
...
和以前的例子不同的是，每个派生类在构造方法中都通过 `super()` 调用了父类的构造方法。另外，当我们在构造方法体内部访问 `this` 之前都调用了 `super()`，这是 TypeScript 强制要求的一个规则。
...
当一个成员被声明为 `private` 的时候，它不能从包含它的类的外部进行访问，例如。
<!--JavaScript-->
...
但是当类型包含 private 和 protected 成员时，处理方式就不一样了。假设两个类型是兼容的，如果其中一个有 private 成员，那么另外一个也必须有同样的 private 成员，对于 protected 成员来说也是一样。
...
例子中有 Animal 和 Rhino 两个类，Rhino 是 Animal 的子类。例子中还有一个看上去 `Shape` 和 Animal 差不多的 Employee 类，我们创建这些类的实例，然后用这些实例彼此进行赋值，看看会发生什么。因为 Animal 和 Rhino 共有 private 声明，所以它们是兼容的。但是 Employee 并不是这样，当我们试图将 Employee 的实例赋值给由 Animal 声明的变量时，会返回类型不兼容的错误。虽然 Employee 也有一个 private 修饰的属性 `name`，但它不是我们在 Animal 中声明的那个 `name`。
...
构造方法也可以使用 `protected` 修饰符，这也就是说不能再类的外部实例化类的实例，但是在子类中可以。例如。
<!--JavaScript-->
...
在上面的例子中，我们在 `Octopus` 类中声明了只读成员 `name` 和带有 `theName` 参数的构造方法，并想使构造方法在执行后 `theName` 始终有值。`参数属性` 可以做到这一点，下面的例子使用参数属性对 `Octopus` 类进行进一步的扩展。
<!--JavaScript-->
...
参数属性可以在构造方法上使用访问修饰符或者 `readonly`，或者二者一起使用。对参数属性使用 private 可以声明和初始化private成员，其他是一样。
...
代码允许使用者随意地设置 `fullName` 的值，我们也可以在对 `fullName` 设置值时加上一些限制。
...
第一，要求你必须设置编译器的输出为 ES5 或者更高，ES3 是不支持的。第二，有 get 而没有 set 的成员将自动被识别为 readonly 成员，当通过代码生成 `.d.ts` 文件时，这很有用。因为使用者无法对成员的值进行修改。

## 静态属性

到现在，我们都仅仅在讨论类的实例化成员，这些都是在类实例化时发生的。我们也可以为类创建静态成员，这些成员对类自身来说是可见的。在这个例子中，我们在 origin 上使用 `static` 修饰符，origin 是所有 grid 的属性，每个实例都通过类名加属性名访问 origin，这看上去和前面加 `this.` 有点类似。在实例中访问可以使用 `this`，而访问静态属性，需要使用类名。
<!--JavaScript-->
...
在示例中，greeter1 的工作方式与之前类似。我们实例化Greeter类，并使用此对象，这是我们以前见过的。

接下来，我们创建一个名为 greeterMaker 的新的变量。这个变量将保存类本身，也就是用另一种方式表示其构造函数。在这里，我们使用 typeof Greeter，即“给我 Greeter 类本身的类型”，而不是实例类型。或者更确切地说，“给我称为 Greeter 的符号的类型”，这是构造函数的类型。该类型将包含 Greeter 的所有静态成员以及创建 Greeter 类实例的构造函数。我们通过在 greeterMaker 上使用new，创建 Greeter 的新实例并像之前一样调用实例方法。
